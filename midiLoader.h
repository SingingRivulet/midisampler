...
inline void loadMidi(T& self, const std::string& str) {
    ...
    for (int track = 0; track < tracks; track++) {
        ...
        auto& t = midifile[track];

        for (int event = 0; event < midifile[track].size(); event++) {
            if (midifile[track][event].isNoteOn() && midifile[track][event].size() >= 3) {
                int position = midifile[track][event].tick;
                int duration = midifile[track][event].getTickDuration();
                int durationS = midifile[track][event].getDurationInSeconds();
                ...
                self.addNote(position, tone, duration, v, infoBuf, instrumentId);
                ...
                self.metaContent.insert({position, std::make_tuple(content, track)});
                //std::cout << position << " " << midifile[track][event].getMetaContent() << std::endl;
            }
        }
    }
